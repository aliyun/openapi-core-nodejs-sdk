...
function replaceRepeatList(params, key, repeat) {
...
function replaceAllLists(params) {
  var dupParams = Object.assign({}, params);
  var keys = Object.keys(params);
  for (let i = 0; i < keys.length; i++) {
    var key = keys[i];
    var value = params[key];
    if (Array.isArray(value)) {
      replaceRepeatList(dupParams, key, value);
    }
  }
  return dupParams;
}
...
function normalize(params) {
  var list = [];
  var flatParams = replaceAllLists(params);
  var keys = Object.keys(flatParams).sort();
  for (let i = 0; i < keys.length; i++) {
    var key = keys[i];
    var value = flatParams[key];
