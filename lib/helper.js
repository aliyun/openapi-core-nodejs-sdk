...
var ALICLOUD_ACCESS_KEY = 'ALICLOUD_ACCESS_KEY';
...
function _formEnvVariables(){
  ...
  envVariables[ALICLOUD_ACCESS_KEY] = 'accessKeyId';
  envVariables[ALICLOUD_SECRET_KEY] = 'accessKeySecret';
  return envVariables;
}
...
function _combindConfigValues(envConfig, config){
  ...
  toBeCombinedValues.forEach(function(key) {
    if(!config[key] && envConfig && envConfig[key]){
